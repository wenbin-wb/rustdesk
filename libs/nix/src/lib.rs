...
// Vendored for the HarmonyOS port. Cargo.toml trims the default feature set because
// several nix modules build against glibc-only symbols (mq_*, aio_*, __fsword_t,
// ST_RELATIME, XFS_SUPER_MAGIC, FDPIC_FUNCPTRS, UNAME26, SigevNotify::SigevThreadId)
// that HarmonyOS' musl-shaped libc does not export, so nix cannot be compiled for
// aarch64-unknown-linux-ohos as published. Trimming leaves the imports and bindings of
// feature-gated code paths unused. Upstream guards against that with the
// `not(feature = "default")` line above, but we keep the `default` feature *name* while
// trimming its contents, so that guard cannot fire. Relax the lint here rather than
// editing every affected module.
#![allow(unused)]
