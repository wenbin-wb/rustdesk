...
| **V13** | 设备页点"清空" | **列表真的清空**（重开应用仍为空） | 此前是并发逐个删除，会互相覆盖导致"删了还在"；现改为一次性写入空列表 |
| **V14** | 设备页删一个设备 → 切到"远程" | 该设备**也从"最近"消失** | 此前删除只刷新自己那个页签 |
| **V15** | 同一设备连续两次点连接 | 第二次弹密码框时**输入框是空的** | 此前重开会保留上次输入的密码 |
| **V16** | 设置页"服务器与中继" | **不出现两次"当前服务器"** | 此前动作行误用了上方信息行的标题当副标题 |
